...
class FinanceCalculator:
    @staticmethod
    def calculate_sale_values(quantidade: int, preco_unitario: float) -> tuple[float, float]:
        valor_total = quantidade * preco_unitario
        lucro_bruto = valor_total
        return valor_total, lucro_bruto
    ...
    def indicators_from_data(sales: list[dict], expenses: list[dict], operational_costs: list[dict]) -> Indicators:
        ...
        lucro_bruto = sum(float(row["lucro"]) for row in sales)
        gastos_totais = sum(float(row["valor"]) for row in expenses)
        custos_totais = sum(float(row["valor"]) for row in operational_costs)
        lucro_liquido = lucro_bruto - gastos_totais - custos_totais
        margem_media = (lucro_liquido / receita_total * 100) if receita_total > 0 else 0.0
        return Indicators(
